...
export function NewQuote() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      ...
      setQuoteId(generateId('MRT', data.quotes.map(q => q.id)));
      setItems([{ seq: 1, desc: '', mat: '', hsn: '40169930', qty: 1, uom: 'pcs', unitPrice: 0, gst: 18, total: 0, packing: '', packingType: '' }]);
    }
  ...
  const addItem = () => setItems([...items, { seq: items.length + 1, desc: '', mat: '', hsn: '40169930', qty: 1, uom: 'pcs', unitPrice: 0, gst: 18, total: 0, rateAsPerWeight: '', rateOverride: false, rateText: '', packing: '', packingType: '' }]);
                  ...
                  <table className="w-full border-collapse border border-g400 text-[12px]">
                    <thead className="bg-g100">
                      <tr>
                        ...
                        <th className="font-mono text-[8px] tracking-[1px] uppercase text-g500 px-3 py-1.5 text-left border border-g400 w-24">Packing</th>
                        <th className="font-mono text-[8px] tracking-[1px] uppercase text-g500 px-3 py-1.5 text-left border border-g400 w-28">Packing Type</th>
                    ...
                    <tbody>
                      {items.map((item, idx) => (
                        <tr key={item.seq} className="hover:bg-g50/50">
                          ...
                          <td className="px-3 py-[5px] border border-g400 align-middle">
                            <input type="text" value={item.packing || ''} placeholder="Packing" onChange={e => updateItem(idx, 'packing', e.target.value)} className="w-full bg-transparent outline-none text-[12px] font-sans text-blk placeholder:text-g300" />
                          </td>
                          <td className="px-3 py-[5px] border border-g400 align-middle">
                            <input type="text" value={item.packingType || ''} placeholder="Packing Type" onChange={e => updateItem(idx, 'packingType', e.target.value)} className="w-full bg-transparent outline-none text-[12px] font-sans text-blk placeholder:text-g300" />
                          </td>
                          <td className="px-3 py-[5px] border border-g400 align-middle">
