...
interface Milestone {
  key: string;
  label: string;
  sub: string;       // congratulatory sub-line
  emoji: string;
...
const STORAGE_KEY = 'mrt_milestones_fired';
...
function isFiredRecently(key: string): boolean {
  const f = getFired();
  return !!f[key] && Date.now() - f[key] < TTL_MS;
}
...
function fireConfetti(tier: Milestone['tier']) {
  ...
  if (tier === 'bronze') {
    ...
    confetti({ particleCount: 90, angle: 60,  spread: 55, origin: { x: 0 }, colors });
    confetti({ particleCount: 90, angle: 120, spread: 55, origin: { x: 1 }, colors });
  } else {
    // diamond — continuous shower
...
export function MilestoneConfetti() {
  ...
  useEffect(() => {
    ...
    const unlocked = MILESTONES.filter(m => !isFiredRecently(m.key) && m.check(stats));
    if (unlocked.length === 0) return;

    const top = unlocked[unlocked.length - 1];
    unlocked.forEach(m => markFired(m.key));
  ...
  }, [data.enquiries.length, data.quotes.length, data.orders.length]);
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] flex items-center justify-center transition-all duration-400 ${visible ? 'bg-black/60 backdrop-blur-sm' : 'bg-black/0 pointer-events-none'}`}
      onClick={dismiss}
    >
      <div
        onClick={e => e.stopPropagation()}
        className={`relative w-[420px] max-w-[90vw] rounded-2xl bg-gradient-to-b ${s.bg} border ${s.ring} shadow-2xl px-8 py-8 flex flex-col items-center text-center transition-all duration-400 ${visible ? 'scale-100 opacity-100 translate-y-0' : 'scale-90 opacity-0 translate-y-6'}`}
      >
        {/* Close */}
        <button type="button" onClick={dismiss} title="Dismiss" aria-label="Dismiss" className="absolute top-3 right-3 w-7 h-7 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white/60 hover:text-white transition-colors">
          <X size={13} />
        </button>

        {/* Tier badge */}
        <span className={`font-mono text-[9px] font-bold tracking-[2.5px] uppercase px-3 py-1 rounded-full ${s.badge} mb-4`}>
          {banner.tier} milestone
        </span>

        {/* Emoji */}
        <div className="text-6xl mb-3 drop-shadow-lg select-none">{banner.emoji}</div>

        {/* Heading */}
        <div className="text-white font-bold text-[22px] leading-tight mb-2">{banner.label}</div>

        {/* Sub */}
        <div className="text-white/60 text-[13px] leading-relaxed mb-6">{banner.sub}</div>

        {/* Stars row */}
        <div className="flex items-center gap-1 mb-6">
          {Array.from({ length: banner.tier === 'bronze' ? 1 : banner.tier === 'silver' ? 2 : banner.tier === 'gold' ? 3 : 4 }).map((_, i) => (
            <svg key={i} width="18" height="18" viewBox="0 0 20 20" fill={s.star}><path d="M10 1l2.6 5.3 5.9.8-4.3 4.1 1 5.8L10 14.3l-5.2 2.7 1-5.8L1.5 7.1l5.9-.8z"/></svg>
          ))}
        </div>
        ...
        <button
          type="button"
          onClick={dismiss}
          className="px-8 py-2.5 rounded-lg bg-white/10 hover:bg-white/20 text-white font-mono text-[11px] font-bold tracking-widest uppercase transition-colors border border-white/20"
        >
          Keep Going 🚀
        </button>
