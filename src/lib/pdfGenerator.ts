...
export function generatePIPDF(
  ...
  doc.text('Sub-Total (excl. GST)', rx - 58, y); doc.setFont('helvetica', 'bold'); doc.setTextColor(30, 30, 30);
  ...
  if (preLines.length > 0) {
    ...
    doc.setFont('helvetica', 'normal'); doc.setTextColor(80, 80, 80);
    doc.text('Taxable Value', rx - 58, y); doc.setFont('helvetica', 'bold'); doc.setTextColor(30, 30, 30);
    doc.text(fmtAmount(taxableValue, sym), rx, y, { align: 'right' }); y += 5.5;
  }

  // GST is charged on the (P&F-inclusive) taxable value.
  doc.setFont('helvetica', 'normal'); doc.setTextColor(80, 80, 80);
  doc.text('GST Amount', rx - 58, y); doc.setFont('helvetica', 'bold'); doc.setTextColor(30, 30, 30);
